...
$(function(){
		...
		var hourDraw = function(){
				...
				for(var i = 0;i<curMonthDays+12;i++){
					if(i%7==0){
						hourEm += '<tr>';
					}
				...
				success:function(data){
					for(var i in data){
						for(var j in data[i].list){
							$('td[data='+data[i].date+']').append(data[i].list[j].type==0?'<a class="label label-primary notSed visible-lg-block" type="0"><i class="glyphicon"></i> '+data[i].list[j].fieldName+'</a>':'<a class="label label-danger visible-lg-block"><i class="glyphicon glyphicon-remove" type="1"></i> ' +data[i].list[j].fieldName+'</a>');
							$('td[data='+data[i].date+']').append('<span class="tag visible-xs-block" type="0">'+data[i].size+'</span>');
							console.log(data[i].list[j].fieldName);
							$("td").tooltip({
								trigger:'hover',
								html:true,
