					...
					for(var i in data){
						...
						for(var j in data[i].list){
							!(function(i){
								...
								//$('td[data='+date+']').find('.row').append('<span class="col-xs-10 col-sm-10 label label-primary">'+fieldName+'</span>');
								$('td[data='+date+']').live('click',function(){
									var _self = $(this);
									var x = _self.offset();
									var w = _self.outerWidth();
									var h = _self.outerHeight();
									$('.layer .row').append('<span class="col-xs-10 col-sm-10 label label-primary">'+fieldName+'</span>');
